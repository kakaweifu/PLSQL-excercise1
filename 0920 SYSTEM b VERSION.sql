...
CREATE OR REPLACE PACKAGE BODY SYSTEM_B_IMPORT_EXPORT AS
    ...
    FROM
        ATTENDANCES A
        LEFT JOIN WORK_SCHEDULES WS ON A.EMPLOYEE_ID = WS.EMPLOYEE_ID AND A.ATTENDANCE_DATE = WS.ATTENDANCE_DATE
        LEFT JOIN TIMECARDS TC ON A.EMPLOYEE_ID = TC.EMPLOYEE_ID AND A.ATTENDANCE_DATE = TC.ATTENDANCE_DATE
    WHERE
        A.ATTENDANCE_DATE BETWEEN I_DATE_FROM AND I_DATE_TO;

    -- デバッグ情報を出力する手続き
    PROCEDURE DEBUG_PRINT(I_MSG IN VARCHAR2) IS
    BEGIN
        IF GC_DEBUG THEN
    ...
    PROCEDURE CREATE_DIRECTORY(I_DIR_NAME IN VARCHAR2) IS
    BEGIN
        LOOP
            BEGIN
    ...
    -- ファイル名を生成する関数
    FUNCTION CREATE_FILE_NAME(P_PREFIX IN VARCHAR2) RETURN VARCHAR2 IS
    BEGIN
        RETURN P_PREFIX || TO_CHAR(SYSDATE, 'YYYYMMDDHH24MI') || '.csv';
    END CREATE_FILE_NAME;

    -- ファイルを開く手続き
    PROCEDURE OPEN_FILE(I_FILE_NAME IN VARCHAR2, I_OPEN_MODE IN VARCHAR2) IS
    BEGIN
    ...
    -- ファイルに行を書き込む手続き
    PROCEDURE PRINT_LINE(I_BUF IN VARCHAR2) IS
    BEGIN
    ...
    -- CSVの項目を取得する関数
    FUNCTION GET_CSV_ITEM(I_CSV IN VARCHAR2, IO_START_POS IN OUT PLS_INTEGER) RETURN VARCHAR2 IS
    ...
    -- 社員IDを変換する関数
    FUNCTION CONVERT_EMPLOYEE_ID(P_EMPLOYEE_ID_A IN VARCHAR2) RETURN VARCHAR2 IS
    ...
    BEGIN
        ...
        IF NOT REGEXP_LIKE(V_TRIMMED_ID, '^[0-9]+$') THEN
            RAISE_APPLICATION_ERROR(-20006, '無効な社員ID形式: ' || V_TRIMMED_ID);
        END IF;
        ...
        RETURN 'H' || V_PADDED_ID || V_CHECK_DIGIT;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20007, '社員ID変換エラー: ' || SQLERRM || ' (ID: ' || P_EMPLOYEE_ID_A || ')');
    END CONVERT_EMPLOYEE_ID;

    -- CSVデータを社員レコードに変換する関数
    FUNCTION TO_EMPLOYEE(I_CSV IN VARCHAR2) RETURN EMPLOYEES%ROWTYPE IS
    ...
    -- 社員データをマージする手続き
    PROCEDURE MERGE_EMPLOYEE(I_E IN EMPLOYEES%ROWTYPE) IS
        V_CONVERTED_ID VARCHAR2(8);
    BEGIN
        V_CONVERTED_ID := CONVERT_EMPLOYEE_ID(I_E.EMPLOYEE_ID);
        
        MERGE INTO EMPLOYEES E
        USING (SELECT V_CONVERTED_ID AS EMPLOYEE_ID,
        ...
        WHEN MATCHED THEN
            UPDATE SET E.FAMILY_NAME = I.FAMILY_NAME,
                       E.GIVEN_NAME = I.GIVEN_NAME,
                       E.IS_VALID = I.IS_VALID,
                       E.LAST_UPDATE_DATE = SYSTIMESTAMP
        WHEN NOT MATCHED THEN
            INSERT (EMPLOYEE_ID, FAMILY_NAME, GIVEN_NAME, IS_VALID, CREATION_DATE, LAST_UPDATE_DATE)
            VALUES (I.EMPLOYEE_ID, I.FAMILY_NAME, I.GIVEN_NAME, I.IS_VALID, SYSTIMESTAMP, SYSTIMESTAMP);
    END MERGE_EMPLOYEE;

    -- CSVデータを勤怠レコードに変換する関数
    FUNCTION TO_ATTENDANCE(I_CSV IN VARCHAR2) RETURN ATTENDANCES%ROWTYPE IS
    ...
    BEGIN
        ...
        L_WORK_SCHEDULE.SCHEDULED_START_TIME := TO_CHAR(TO_DATE(L_TIME_STR, 'HH24MI'), 'HH24:MI');
        L_TIME_STR := GET_CSV_ITEM(I_CSV, L_START_POS);
        L_WORK_SCHEDULE.SCHEDULED_END_TIME := TO_CHAR(TO_DATE(L_TIME_STR, 'HH24MI'), 'HH24:MI');

        L_TIME_STR := GET_CSV_ITEM(I_CSV, L_START_POS);
        L_TIMECARD.START_TIME := TO_CHAR(TO_DATE(L_TIME_STR, 'HH24MI'), 'HH24:MI');
        L_TIME_STR := GET_CSV_ITEM(I_CSV, L_START_POS);
        L_TIMECARD.END_TIME := TO_CHAR(TO_DATE(L_TIME_STR, 'HH24MI'), 'HH24:MI');
...
L_ATTENDANCE.MIDNIGHT_WORKING_MINUTES := TO_NUMBER(NVL(GET_CSV_ITEM(I_CSV, L_START_POS), '0'));
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DEBUG_PRINT('TO_ATTENDANCEでエラー発生: ' || SQLERRM);
            RAISE_APPLICATION_ERROR(-20005, '勤怠データの解析中にエラーが発生しました: ' || SQLERRM || ' (行: ' || I_CSV || ')');
    END TO_ATTENDANCE;
    ...
    BEGIN
        ...
        USING (SELECT V_CONVERTED_ID AS EMPLOYEE_ID,
               ...
               FROM DUAL) I
        ON (A.EMPLOYEE_ID = I.EMPLOYEE_ID AND A.ATTENDANCE_DATE = I.ATTENDANCE_DATE)
        WHEN MATCHED THEN
            UPDATE SET A.ATTENDANCE_TYPE_CODE = I.ATTENDANCE_TYPE_CODE,
                       A.WORKING_MINUTES = I.WORKING_MINUTES,
                       A.ABSENCE_MINUTES = I.ABSENCE_MINUTES,
                       A.OVER_TIME_MINUTES = I.OVER_TIME_MINUTES,
                       A.MIDNIGHT_WORKING_MINUTES = I.MIDNIGHT_WORKING_MINUTES,
                       A.OVER_60_OVER_TIME_MINUTES = I.OVER_60_OVER_TIME_MINUTES,
                       A.LAST_UPDATE_DATE = SYSTIMESTAMP
        WHEN NOT MATCHED THEN
            INSERT (EMPLOYEE_ID, ATTENDANCE_DATE, ATTENDANCE_TYPE_CODE, WORKING_MINUTES, 
                    ABSENCE_MINUTES, OVER_TIME_MINUTES, MIDNIGHT_WORKING_MINUTES, 
                    OVER_60_OVER_TIME_MINUTES, CREATION_DATE, LAST_UPDATE_DATE)
            VALUES (I.EMPLOYEE_ID, I.ATTENDANCE_DATE, I.ATTENDANCE_TYPE_CODE, I.WORKING_MINUTES,
                    I.ABSENCE_MINUTES, I.OVER_TIME_MINUTES, I.MIDNIGHT_WORKING_MINUTES,
                    I.OVER_60_OVER_TIME_MINUTES, SYSTIMESTAMP, SYSTIMESTAMP);
        ...
        IF I_WORK_SCHEDULE.SCHEDULED_START_TIME IS NOT NULL OR I_WORK_SCHEDULE.SCHEDULED_END_TIME IS NOT NULL THEN
            ...
            USING (SELECT V_CONVERTED_ID AS EMPLOYEE_ID,
                   ...
                   FROM DUAL) I
            ON (WS.EMPLOYEE_ID = I.EMPLOYEE_ID AND WS.ATTENDANCE_DATE = I.ATTENDANCE_DATE)
            WHEN MATCHED THEN
                UPDATE SET WS.SCHEDULED_START_TIME = I.SCHEDULED_START_TIME,
                           WS.SCHEDULED_END_TIME = I.SCHEDULED_END_TIME,
                           WS.BREAKING_MINUTES = I.BREAKING_MINUTES,
                           WS.LAST_UPDATE_DATE = SYSTIMESTAMP
            WHEN NOT MATCHED THEN
                INSERT (EMPLOYEE_ID, ATTENDANCE_DATE, SCHEDULED_START_TIME, SCHEDULED_END_TIME, 
                        BREAKING_MINUTES, CREATION_DATE, LAST_UPDATE_DATE)
                VALUES (I.EMPLOYEE_ID, I.ATTENDANCE_DATE, I.SCHEDULED_START_TIME, I.SCHEDULED_END_TIME,
                        I.BREAKING_MINUTES, SYSTIMESTAMP, SYSTIMESTAMP);
        END IF;

        -- Merge TIMECARDS
        IF I_TIMECARD.START_TIME IS NOT NULL OR I_TIMECARD.END_TIME IS NOT NULL THEN
            MERGE INTO TIMECARDS TC
            USING (SELECT V_CONVERTED_ID AS EMPLOYEE_ID,
                          I_ATTENDANCE.ATTENDANCE_DATE AS ATTENDANCE_DATE,
                          I_TIMECARD.START_TIME AS START_TIME,
                          I_TIMECARD.END_TIME AS END_TIME
                   FROM DUAL) I
            ON (TC.EMPLOYEE_ID = I.EMPLOYEE_ID AND TC.ATTENDANCE_DATE = I.ATTENDANCE_DATE)
            WHEN MATCHED THEN
                UPDATE SET TC.START_TIME = I.START_TIME,
                           TC.END_TIME = I.END_TIME,
                           TC.LAST_UPDATE_DATE = SYSTIMESTAMP
            WHEN NOT MATCHED THEN
                INSERT (EMPLOYEE_ID, ATTENDANCE_DATE, START_TIME, END_TIME, CREATION_DATE, LAST_UPDATE_DATE)
                VALUES (I.EMPLOYEE_ID, I.ATTENDANCE_DATE, I.START_TIME, I.END_TIME, SYSTIMESTAMP, SYSTIMESTAMP);
        END IF;
                ...
                LOOP
                    ...
                    EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                            DEBUG_PRINT(V_EMP_LINE_COUNT || '行の社員データを読み込みました。');
                            EXIT;
                    END;
                END LOOP;
            ELSE
                RAISE_APPLICATION_ERROR(-20001, '"' || I_EMPLOYEES_FILE || '"は社員マスタのCSVファイルではありません。');
            END IF;
        EXCEPTION
            WHEN OTHERS THEN
                DEBUG_PRINT('社員マスタファイル読み込み中にエラーが発生しました: ' || SQLERRM);
                RAISE;
        END;
        CLOSE_FILE;
                ...
                LOOP
                    BEGIN
                        ...
                        BEGIN
                            ...
                            ELSE
                                DEBUG_PRINT('無効なレコードをスキップします。行番号: ' || V_ATT_LINE_COUNT);
                            END IF;
                        END;
                    EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                            DEBUG_PRINT(V_ATT_LINE_COUNT || '行の勤怠データを読み込みました。');
                            EXIT;
                    END;
                END LOOP;
            ELSE
                RAISE_APPLICATION_ERROR(-20002, '"' || I_ATTENDANCES_FILE || '"は勤怠データのCSVファイルではありません。');
            END IF;
        EXCEPTION
            WHEN OTHERS THEN
                DEBUG_PRINT('勤怠データファイル読み込み中にエラーが発生しました: ' || SQLERRM);
                RAISE;
        END;
        CLOSE_FILE;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DEBUG_PRINT('エラーが発生しました。ロールバックします。');
            DEBUG_PRINT('エラーコード: ' || SQLCODE);
            DEBUG_PRINT('エラーメッセージ: ' || SQLERRM);
            ROLLBACK;
            CLOSE_FILE;
            DROP_DIRECTORY;
            RAISE;
...
BEGIN
    ...
    FOR R IN (
        ...
        DBMS_OUTPUT.PUT_LINE('EMPLOYEESテーブルのレコード数: ' || R.EMP_COUNT);
    END LOOP;
    ...
    FOR R IN (
        ...
        DBMS_OUTPUT.PUT_LINE('ATTENDANCESテーブルのレコード数: ' || R.ATT_COUNT);
    END LOOP;
    ...
    FOR R IN (
        ...
        DBMS_OUTPUT.PUT_LINE('WORK_SCHEDULESテーブルのレコード数: ' || R.WS_COUNT);
    END LOOP;
    ...
    FOR R IN (
        ...
        DBMS_OUTPUT.PUT_LINE('TIMECARDSテーブルのレコード数: ' || R.TC_COUNT);
    END LOOP;
